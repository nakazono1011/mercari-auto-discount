...
class WeeklyCommentCrawler(BaseCrawler):
    ...
    def _comment_all(self, target_urls):
        for target_url in target_urls:
            try:
                ...
                item_name = self.driver.find_element(By.CSS_SELECTOR, 'mer-heading[data-testid="name"]').text
                
                self.driver.find_element(By.CSS_SELECTOR, ".input-node").send_keys(config.WEEKLY_SALE_COMMENT)
                
                self.driver.find_element(By.CSS_SELECTOR, "mer-button[data-location='item_details:comment:post_button']").click()
    ...
    def _scrape_target_urls(self):
        ...
        for el in item_list:
            ...
            if like_count < self.MIN_LIKE_COUNT: continue
